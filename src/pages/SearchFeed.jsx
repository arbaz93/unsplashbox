...
export default function SearchFeed() {
  ...
  return (
    <section className=" flex-col pb-16 flex items-center min-height-equal-vh-minus-nav-footer">
      <div className="py-16 px-9 w-full flex justify-center half-gradient">
        <SearchForm text={searchQuery} />
      </div>
      {fetchStatus === 'fetching' ? spinner
        : fetchStatus === 400 ||  fetchStatus === 401 || fetchStatus === 404 || fetchStatus === 505 || fetchStatus === 503 ? <p>{fetchStatus} | Something went wrong!</p>
        : fetchStatus === 403 ? <p>{fetchStatus} | demo limit reached</p>
        : (fetchStatus === 'empty' && images.length === 0) ? <p>{searchQuery} not found!</p>
          : <div className="w-[80%]"><ImageGrid images={images} callback={fetchImages} fetchStatus={fetchStatus} /></div>
      }
    </section>
  )
}

...
export default function CollectionItem({ collectionData }) {
  ...
  return (
    <div className='flex gap-4 items-center duration-200 hover:bg-ntrl-clr-100 p-4 rounded-[0.5rem]'>
      <div className='w-20 aspect-square rounded-[0.25rem]'>
        <img className='rounded-[0.25rem] object-cover w-full h-full ' src={collectionData?.urls?.regular} alt="" />
      </div>
      <div className='flex flex-col gap-2 justify-center flex-1'>
        <Link to={''}>
          <HeadingSmall text="Collection Name" />
        </Link>
        <Description text="20 photos" size='mid' />
      </div>
      <div>

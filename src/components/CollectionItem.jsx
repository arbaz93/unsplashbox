...
import { HeadingSmall, Description, Spinner } from '.';
import { addToCollection, removefromCollection } from '../js/handleCollectionsAPI';
import { useState } from 'react';
import { useStore, useAccessStore, useCollectionsStore } from '../zustandstore/store';
...
export default function CollectionItem({ data, photoId, actionType, setCollectionsBelongingToImage }) {
  ...
  function handleClick() {
    ...
    if (actionType === 'remove') {
      removefromCollection(data.id, photoId, accessToken.access_token)
        .then(() => {
          setCollectionsBelongingToImage(prev => {
            const updatedCollections = prev.filter(collection => collection.id != data.id);
            console.log(updatedCollections)
            return updatedCollections;
          })
          setResponseMessage({ message: 'Image Removed Successfully', type: 'success' })
        })
        .catch(err => {
  ...
  return (
    <div className='flex gap-4 items-center duration-200 hover:bg-ntrl-clr-100 rounded-[0.5rem]'>
      ...
      <div>
        {isAdded &&
          <div className=' px-4'><Spinner /></div>}

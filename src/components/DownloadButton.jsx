...
export default function DownloadButton({ text, imageLink, filename }) {
    ...
    async function downloadImage() {
        setResponseMessage({message: 'Image downloading!', type: 'success'})
        try {
            const response = await axios.get(`${imageLink}?client_id=${API_KEY}`);
            const url = await response.data.url;
            
            const imageResponse = await axios.get(url, {responseType: 'blob'});
            const blobUrl = window.URL.createObjectURL(imageResponse.data);
            ...
            setResponseMessage({message: 'Something went wrong!', type: 'error'})
            console.error(err)
        }
